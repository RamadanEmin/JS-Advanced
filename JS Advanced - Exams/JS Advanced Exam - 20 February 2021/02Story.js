class Story {
    ...
    comment(username, content, id) {
        const findedComents = this._comments.find(c => c.id === id);
        if (!findedComents || id === undefined) {
            this._comments.push({ id: this._comments.length + 1, username, content, replies: [] });
            return `${username} commented on ${this.title}`;
        }
        findedComents.replies.push({ id: `${findedComents.id}.${findedComents.replies.length + 1}`, username, content });
